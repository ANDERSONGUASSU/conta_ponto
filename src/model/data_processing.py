...
def processing_district_data(data, data1):
    ...
    df_district = pd.DataFrame(district_columns,
                               columns=['Distrito', 'Mat/Carteiro',
                                        'Qnt_Objetos',
                                        'Vazia', 'Ar', 'Vazia1',
                                        'Num_lista', 'Impressão'])
    ...
    df_district.to_sql('dados_distritos', conn,
    ...
    df_object = pd.DataFrame(
        object_columns,
        columns=['Num_lista', 'objeto', 'false',
                 'endereço', 'ar', 'cep', 'zero'])
    ...
    distrito_dict = dict(zip(df_district["Num_lista"],
                             df_district["Distrito"]))
    ...
    for key, values in object_dict.items():
        ...
        cursor.execute(f"CREATE TABLE IF NOT EXISTS {table_name} (objeto TEXT,endereco TEXT)") # noqa

        for value in values:
            cursor.execute(f"INSERT INTO {table_name} (objeto, endereco) VALUES (?, ?)", (value[0], value[1])) # noqa
    conn.commit()
